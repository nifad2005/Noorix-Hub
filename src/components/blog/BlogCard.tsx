...
import { CalendarDays, Tag, FileText, Pencil, Trash2 } from 'lucide-react';
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Blog post "${blog.title}" has been deleted.`,
      });
      if (onBlogDeleted) {
    ...
    <Card className="h-full flex flex-col md:flex-row overflow-hidden hover:shadow-xl transition-shadow duration-300 ease-in-out group relative">
      ...
      <div className="flex flex-col flex-grow">
        <CardHeader className="p-4 pb-2">
          {blog.category && (
            <Badge variant="secondary" className="text-xs uppercase w-fit mb-1">{blog.category}</Badge>
          )}
          <Link href={`/blogs/${blog._id}`} passHref className="block">
