...
export function ContentHandleCard({ handle, onHandleDeleted, isSelected, onSelectionChange }: ContentHandleCardProps) {
  const { toast } = useToast();
  const [isDeleting, setIsDeleting] = useState(false);

  const handleDelete = async (e?: React.MouseEvent) => {
    e?.stopPropagation(); // Prevent card selection when initiating delete
    setIsDeleting(true);
    try {
